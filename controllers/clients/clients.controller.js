  ...
  try {
    ...
    if (Firstname.length < 3 || Lastname.length < 3) {
      return res.status(400).json({
        success: false,
        message:
          "Both Firstname and lastname must be at least 3 characters long",
      });
    }
    if (Firstname.length > 30 || Lastname.length > 30) {
      return res.status(400).json({
        success: false,
        message:
          "Both Firstname and lastname must be less than 30 characters long",
      });
    }
...
exports.resendCode = async (req, res) => {
  ...
  if (!req.body.Telephone.match(Phonepattern)) {
    return res.json({
      success: false,
      message: "Invalid telephone number",
    });
  }
  if (!req.body.Telephone.startsWith("+")) {
    return res.status(400).json({
      success: false,
      message: "Include country code in phone number.",
    });
...
exports.checkingPhone = async (req, res) => {
  try {
    ...
    if (!phoneNumber.match(Phonepattern)) {
      return res.json("invalid telephone number");
    }
    if (code.length !== 6) {
      return res.status(400).json({
        success: false,
      ...
      twilio.verify
        ...
        .then((data) => {
          if (data.status == "approved") {
            client.query(
              `UPDATE clients set verified=true where telephone='${phoneNumber}'`,
              (error, result) => {
                if (error) console.log(error.message);
              }
            );
            client.query(
              `SELECT * FROM clients where telephone='${phoneNumber}'`,
              async (error, result) => {
                if (error) console.log(error);
                else {
...
exports.forgotPassword = async (req, res, next) => {
  ...
  try {
    const Phonepattern =
      /^\s*(?:\+?(\d{1,3}))?[-. (]*(\d{3})[-. )]*(\d{3})[-. ]*(\d{4})(?: *x(\d+))?\s*$/;
    if (!Telephone) {
      return res.status(400).json("Phone number must be provided");
    }
    if (!Telephone.match(Phonepattern)) {
...
exports.verifyResetPassCode = (req, res) => {
  ...
  if (!Telephone.match(Phonepattern)) {
    return res.status(400).json("Invalid telephone number");
  }
  if (!code) {
    return res.status(400).json({
      success: false,
...
exports.resetPasssword = async (req, res, next) => {
  const { password, confirmPassword, Telephone } = req.body;
  try {
    const Phonepattern =
      /^\s*(?:\+?(\d{1,3}))?[-. (]*(\d{3})[-. )]*(\d{3})[-. ]*(\d{4})(?: *x(\d+))?\s*$/;
    if (!Telephone) {
    ...
    client.query(
      `update clients set password='${hash}' where telephone='${Telephone}'`,
      (err, result) => {
        if (err) {
          return res.status(400).json({
            success: false,
  ...
  try {
    ...
    client.query(`${query}`, async (err, result) => {
      if (err) console.log(err);
        ...
        await client.query(
          `UPDATE clients set firstname='${Firstname}',lastname='${Lastname}',telephone='${Telephone}' where clientid='${req.user[0].clientid}'`,
          (err, resp) => {
            if (err) {
              console.log(err);
              return res
  ...
  try {
    ...
    client.query(`${query}`, async (err, result) => {
      if (err) console.log(err);
        ...
        await client.query(
          `Delete from clients where clientId='${req.user[0].clientid}'`,
          (err, resp) => {
            if (err) {
              console.log(err);
              return res
...
exports.updatePassword = async (req, res) => {
  ...
  try {
    ...
    client.query(
      `SELECT * FROM clients where clientid='${req.user[0].clientid}'`,
      async (error, result) => {
        if (error) console.log(error.message);
        else {
          if (result.rows.length == 0) {
            ...
            if (!currentPassword || !newPassword || !confirmPassword) {
              return res.status(400).json({
                success: false,
                message:
                  "currrent password, newPassword and confirm password are required",
              });
            }
            ...
            if (!passMatch) {
              ...
              client.query(
                `UPDATE clients set password='${hash}' WHERE clientid='${req.user[0].clientid}'`,
                (error, result) => {
                  if (error) console.log(error.message);
                  return res.status(200).json({
                    success: true,
        ...
        if (error) console.log(error.message);
          ...
          if (result.rows.length == 0) {
            ...
            if (!req.files) {
              ...
              file.name = `photo_${req.user[0].clientid}${
              ...
              file.mv(
                ...
                async (err) => {
                  ...
                  client.query(
                    `UPDATE clients SET photo='${file.name}' WHERE clientid='${req.user[0].clientid}'`,
                    (error, result) => {
                      if (error) console.log(error.message);
                      res.status(200).json({
                        success: true,
